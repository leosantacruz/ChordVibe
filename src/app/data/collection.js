export const list = [
    {
        "title": "Stellar Spirit",
        "thumbnail": "0.webp",
        "progression": [
            {
                "name": "Gm(add9)",
                "notes": ["G2", "Bb2", "D3", "A3"]
            },
            {
                "name": "Ebmaj7",
                "notes": ["Eb3", "G3", "Bb3", "D4"]
            },
            {
                "name": "Fsus2",
                "notes": ["F3", "G3", "C4"]
            },
            {
                "name": "Bb(add13)",
                "notes": ["Bb2", "D3", "G3", "F4"]
            },
            {
                "name": "Gm7",
                "notes": ["G2", "Bb2", "D3", "F3"]
            },
            {
                "name": "Eb(add9)",
                "notes": ["Eb3", "G3", "Bb3", "F4"]
            },
            {
                "name": "F7sus4",
                "notes": ["F2", "Bb2", "C3", "Eb3"]
            },
            {
                "name": "Gm(add9)",
                "notes": ["G2", "Bb2", "D3", "A3"]
            }
        ],
        "description": "Float through the cosmos with this dreamy progression. Perfect for late-night vibes and stargazing feels. Let the stellar spirit guide your ears!",
        "scale": ["G2", "A2", "Cb2", "C3", "D3", "Eb3", "F3"]

    },
    {
        "title": "Fantastic Romance",
        "thumbnail": "1.webp",
        "progression": [
            {
                "name": "Em7",
                "notes": ["E3", "G3", "B3", "D4"]
            },
            {
                "name": "B7/F#",
                "notes": ["F#2", "B2", "D#3", "A3"]
            },
            {
                "name": "Cmaj7",
                "notes": ["C3", "E3", "G3", "B3"]
            },
            {
                "name": "Am(add9)",
                "notes": ["A3", "C4", "E4", "B4"]
            },
            {
                "name": "F#m7b5",
                "notes": ["F#3", "A3", "C4", "E4"]
            },
            {
                "name": "B7",
                "notes": ["B2", "D#3", "F#3", "A3"]
            },
            {
                "name": "Em(add11)",
                "notes": ["E3", "G3", "A3", "B3", "D4"]
            },
            {
                "name": "Dmaj7",
                "notes": ["D3", "F#3", "A3", "C#4"]
            }
        ],
        "description": "Dive into a world of fantasy with this enchanting chord progression. Feel the magic in the air and let your imagination soar with this romantic soundscape."
    },
    {
        "title": "Nostalgic Mist",
        "thumbnail": "2.webp",
        "progression": [
            {
                "name": "Dm",
                "notes": ["D3", "F3", "A3"]
            },
            {
                "name": "Am(add9)",
                "notes": ["A2", "C3", "E3", "B3"]
            },
            {
                "name": "Bbmaj7",
                "notes": ["Bb2", "D3", "F3", "A3"]
            },
            {
                "name": "Gm9",
                "notes": ["G2", "Bb2", "D3", "F3", "A3"]
            },
            {
                "name": "Em7b5",
                "notes": ["E2", "G2", "Bb2", "D3"]
            },
            {
                "name": "A7",
                "notes": ["A2", "C#3", "E3", "G3"]
            },
            {
                "name": "Dm(add11)",
                "notes": ["D2", "F2", "G2", "A2", "C3"]
            },
            {
                "name": "C(add9)",
                "notes": ["C3", "E3", "G3", "D4"]
            }
        ],
        "description": "Wrap yourself in the warmth of memories with this nostalgic progression. Perfect for those chill moments when you're feeling a bit reflective. Get lost in the mist of sound."
    },
    {
        "title": "Cosmic Lounge",
        "thumbnail": "3.webp",
        "progression": [
            {
                "name": "Am9",
                "notes": ["A2", "C3", "E3", "G3", "B3"]
            },
            {
                "name": "D13",
                "notes": ["D2", "F#2", "C3", "E3", "B3"]
            },
            {
                "name": "Gmaj7",
                "notes": ["G2", "B2", "D3", "F#3"]
            },
            {
                "name": "Em7",
                "notes": ["E2", "G2", "B2", "D3"]
            },
            {
                "name": "A7b9",
                "notes": ["A2", "C#3", "E3", "G3", "Bb3"]
            },
            {
                "name": "D7",
                "notes": ["D2", "F#2", "A2", "C3"]
            },
            {
                "name": "Gmaj9",
                "notes": ["G2", "B2", "D3", "F#3", "A3"]
            }
        ],
        "description": "Chill out with this smooth cosmic lounge progression. Perfect for kicking back, sipping something cool, and letting the universe's chill vibes wash over you. Tune in and zone out."
    },
    {
        "title": "Radiant Smile",
        "thumbnail": "4.webp",
        "progression": [
            {
                "name": "C(add9)",
                "notes": ["C3", "E3", "G3", "D4"]
            },
            {
                "name": "G/B",
                "notes": ["B2", "D3", "G3"]
            },
            {
                "name": "Am7",
                "notes": ["A2", "C3", "E3", "G3"]
            },
            {
                "name": "Fmaj7",
                "notes": ["F2", "A2", "C3", "E3"]
            },
            {
                "name": "Dm7",
                "notes": ["D2", "F2", "A2", "C3"]
            },
            {
                "name": "G7sus4",
                "notes": ["G2", "C3", "D3", "F3"]
            },
            {
                "name": "C/E",
                "notes": ["E2", "G2", "C3"]
            },
            {
                "name": "C(add9)",
                "notes": ["C3", "E3", "G3", "D4"]
            }
        ],
        "description": "Let the sunshine in with this bright and cheerful progression. It's like a musical hug that'll instantly boost your mood. Get ready to smile from ear to ear!"
    },
    {
        "title": "Pop Galaxy",
        "thumbnail": "5.webp",
        "progression": [
            {
                "name": "G(add9)",
                "notes": ["G2", "B2", "D3", "A3"]
            },
            {
                "name": "D/F#",
                "notes": ["F#2", "A2", "D3"]
            },
            {
                "name": "Em7",
                "notes": ["E2", "G2", "B2", "D3"]
            },
            {
                "name": "C(add9)",
                "notes": ["C2", "E2", "G2", "D3"]
            },
            {
                "name": "G/B",
                "notes": ["B2", "D3", "G3"]
            },
            {
                "name": "Dsus4",
                "notes": ["D2", "G2", "A2"]
            },
            {
                "name": "Em(add11)",
                "notes": ["E2", "G2", "A2", "B2", "D3"]
            },
            {
                "name": "Cmaj7",
                "notes": ["C2", "E2", "G2", "B2"]
            }
        ],
        "description": "Blast off with this catchy pop progression from another galaxy! Perfect for your next hit or just vibing out. Get ready for an interstellar musical journey."
    },
    {
        "title": "Youth Concert",
        "thumbnail": "6.webp",
        "progression": [
            {
                "name": "E(add9)",
                "notes": ["E3", "F#3", "G#3", "B3"]
            },
            {
                "name": "C#m7",
                "notes": ["C#3", "E3", "G#3", "B3"]
            },
            {
                "name": "F#m9",
                "notes": ["F#2", "A2", "C#3", "E3", "G#3"]
            },
            {
                "name": "B7sus4",
                "notes": ["B2", "E3", "F#3", "A3"]
            },
            {
                "name": "G#m7",
                "notes": ["G#2", "B2", "D#3", "F#3"]
            },
            {
                "name": "A(add9)",
                "notes": ["A2", "B2", "C#3", "E3"]
            },
            {
                "name": "E(add9)",
                "notes": ["E3", "F#3", "G#3", "B3"]
            }
        ],
        "description": "Feel the energy with this vibrant and youthful concert progression. Perfect for that live music feel, bringing the hype and good vibes straight to your ears. Let's rock!"
    },
    {
        "title": "Eclectic Fusion",
        "thumbnail": "7.webp",
        "progression": [
            {
                "name": "Fm7",
                "notes": ["F2", "Ab2", "C3", "Eb3"]
            },
            {
                "name": "Dbmaj7",
                "notes": ["Db2", "F2", "Ab2", "C3"]
            },
            {
                "name": "Gm7b5",
                "notes": ["G2", "Bb2", "Db3", "F3"]
            },
            {
                "name": "C7",
                "notes": ["C3", "E3", "G3", "Bb3"]
            },
            {
                "name": "Abmaj7",
                "notes": ["Ab2", "C3", "Eb3", "G3"]
            },
            {
                "name": "Bb7",
                "notes": ["Bb2", "D3", "F3", "Ab3"]
            },
            {
                "name": "Fm9",
                "notes": ["F2", "Ab2", "C3", "Eb3", "G3"]
            }
        ],
        "description": "Experience a sonic melting pot with this eclectic fusion progression. A mix of flavors that will surprise and delight your ears. Get ready for a unique musical adventure!"
    },
    {
        "title": "Urban Fantasy",
        "thumbnail": "8.webp",
        "progression": [
            {
                "name": "Em(add9)",
                "notes": ["E2", "G2", "B2", "F#3"]
            },
            {
                "name": "Dmaj7/F#",
                "notes": ["F#2", "A2", "C#3", "D3"]
            },
            {
                "name": "Cmaj7",
                "notes": ["C3", "E3", "G3", "B3"]
            },
            {
                "name": "G(add13)",
                "notes": ["G2", "B2", "D3", "E3"]
            },
            {
                "name": "Am9",
                "notes": ["A2", "C3", "E3", "G3", "B3"]
            },
            {
                "name": "B7sus4",
                "notes": ["B2", "E3", "F#3", "A3"]
            },
            {
                "name": "Em(add9)",
                "notes": ["E2", "G2", "B2", "F#3"]
            }
        ],
        "description": "Step into a world where the mystical meets the modern with this urban fantasy progression. Cool vibes for city nights and daydreaming about magic. Feel the urban enchantment!"
    },
    {
        "title": "Orchestral Ecstasy",
        "thumbnail": "9.webp",
        "progression": [
            {
                "name": "Am(add9)",
                "notes": ["A2", "C3", "E3", "B3"]
            },
            {
                "name": "F(add11)",
                "notes": ["F2", "A2", "Bb2", "C3"]
            },
            {
                "name": "Dm(add9)",
                "notes": ["D2", "F2", "A2", "E3"]
            },
            {
                "name": "E7(b9)",
                "notes": ["E2", "G#2", "B2", "D3", "F3"]
            },
            {
                "name": "Dm7b5",
                "notes": ["D2", "F2", "Ab2", "C3"]
            },
            {
                "name": "Fmaj7",
                "notes": ["F2", "A2", "C3", "E3"]
            },
            {
                "name": "Gsus4",
                "notes": ["G2", "C3", "D3"]
            },
            {
                "name": "Am(add9)",
                "notes": ["A2", "C3", "E3", "B3"]
            }
        ],
        "description": "Experience the grandeur with this orchestral ecstasy progression. Big, emotional, and epic, it's perfect for when you want your music to feel larger than life. Prepare for sonic bliss!"
    },
    {
        "title": "Viaje Dórico Medieval",
        "thumbnail": "9.webp",
        "progression": [
            {
                "name": "Dm",
                "notes": ["D3", "F3", "A3"]
            },
            {
                "name": "G",
                "notes": ["G3", "B3", "D4"]
            },
            {
                "name": "Am",
                "notes": ["A3", "C4", "E4"]
            },
            {
                "name": "Dm",
                "notes": ["D3", "F3", "A3"]
            },
            {
                "name": "F",
                "notes": ["F3", "A3", "C4"]
            },
            {
                "name": "C",
                "notes": ["C3", "E3", "G3"]
            },
            {
                "name": "G",
                "notes": ["G3", "B3", "D4"]
            },
            {
                "name": "Am",
                "notes": ["A3", "C4", "E4"]
            },
            {
                "name": "Dm",
                "notes": ["D3", "F3", "A3"]
            }
        ],
        "description": "Transpórtate a la Edad Media con esta progresión en modo dórico. Triadas simples crean una atmósfera modal auténtica, perfecta para evocar el espíritu sereno y místico de la música medieval.",
        "scale": ["C3", "D3", "E3", "F3", "G3", "A3", "B3"]
    },
    {
        "title": "Progresión Melancólica al Estilo Lana Del Rey",
        "thumbnail": "9.webp",
        "progression": [
            {
                "name": "Am",
                "notes": ["A2", "C3", "E3"]
            },
            {
                "name": "Fmaj7",
                "notes": ["F2", "A2", "C3", "E3"]
            },
            {
                "name": "C",
                "notes": ["C3", "E3", "G3"]
            },
            {
                "name": "G7",
                "notes": ["G2", "B2", "D3", "F3"]
            },
            {
                "name": "Dm7",
                "notes": ["D3", "F3", "A3", "C4"]
            },
            {
                "name": "E7",
                "notes": ["E2", "G#2", "B2", "D3"]
            },
            {
                "name": "Am(add9)",
                "notes": ["A2", "C3", "E3", "B3"]
            },
            {
                "name": "Fmaj7",
                "notes": ["F2", "A2", "C3", "E3"]
            }
        ],
        "description": "Sumérgete en la esencia melancólica y cinematográfica de Lana Del Rey con esta progresión de acordes. Con tríadas emotivas y extensiones suaves como séptimas y novenas, esta secuencia evoca un paisaje sonoro nostálgico y etéreo, perfecto para una balada introspectiva o un momento de reflexión bajo luces tenues.",
        "scale": ["A2", "B2", "C3", "D3", "E3", "F3", "G3"]
    }
]